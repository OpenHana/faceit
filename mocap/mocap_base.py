...
from bpy.types import Object, Action, Context, ID
...
class MocapBase:
    ...
    sk_action_slot: int = None
    head_action: Action = None
    head_action_slot: int = None
    eye_action: Action = None
    eye_action_slot: int = None

    mix_mode = 'REPLACE'  # 'MIX'
    ...
    def set_sk_action(self, action, slot: int = None):
        """
        :param action: (bpy.types.Action)
        :param slot: (int) The slot handle of the action
        """
        self.sk_action = action
        if slot is not None:
            self.sk_action_slot = slot

    def set_head_action(self, action: Action, slot: int = None):
        """
        :param action: (bpy.types.Action)
        :param slot: (int) The slot handle of the action
        """
        ...
        if slot is not None:
            self.head_action_slot = slot

    def set_eye_action(self, action: Action, slot: int = None):
        """
        :param action: (bpy.types.Action)
        :param slot: (int) The slot handle of the action
        """
    ...
    def set_shape_targets(self, objects=None, retarget_shapes=None, animate_eye_look_shapes=True, only_eye_look=False):
        ...
        self.dynamic_sk_ranges = bpy.context.preferences.addons[base_package].preferences.dynamic_shape_key_ranges
    ...
    def _anim_values_to_keyframes(self, fc, frames, anim_values):
        ...
        populate_keyframe_points_from_np_array(
            ...
            add=True,
            join_with_existing=self.mix_mode == 'MIX',
            generated=True
        )
    ...
    def recording_to_keyframes(self) -> bool:
        ...
        if self.animate_shapes or self.animate_eye_shapes:
            ...
            if sk_animation_lists:
                ...
                if bpy.app.version >= (4, 4, 0):
                    slot = self.sk_action_slot
                fcurves = get_fcurves_from_slot(self.sk_action, slot)
        ...
        if self.head_obj and (self.animate_head_location or self.animate_head_rotation):
            ...
            slot = None
            if bpy.app.version >= (4, 4, 0):
                slot = self.head_action_slot
            fcurves = get_fcurves_from_slot(self.head_action, slot)
            ...
            if self.animate_head_rotation and head_rot_animation_lists:
                ...
                for i in range(rot_channel_count):
                    ...
                    fc = fcurves.find(head_dp_base + self.head_rotation_data_path, index=i)
                    if not fc:
                        fc = fcurves.new(head_dp_base + self.head_rotation_data_path, index=i)
                    self._anim_values_to_keyframes(fc, self.animation_timestamps, anim_values)
                    keyframes_added = True
            # Head Location
            if self.animate_head_location and head_loc_animation_lists:
                ...
                for i in range(3):
                    ...
                    fc = fcurves.find(head_dp_base + loc_dp, index=i)
                    if not fc:
                        fc = fcurves.new(head_dp_base + loc_dp, index=i)
                    self._anim_values_to_keyframes(fc, self.animation_timestamps, anim_values)
                    keyframes_added = True
            # Eye Rotation
        if self.animate_eye_bones:
            if self.eye_L_bone:
                ...
                slot = None
                if bpy.app.version >= (4, 4, 0):
                    slot = self.eye_action_slot
                fcurves = get_fcurves_from_slot(self.eye_action, slot)
                ...
                for i in range(rot_channel_count_L):
                    ...
                    if self.use_smoothing_eye_bones:
                        if self.smooth_filter_eye_bones == 'SMA':
                            ...
                            anim_values = gaussian_filter1d(anim_values, self.smooth_window_eye_bones, 2)
                    fc = fcurves.find(eye_L_dp, index=i)
                    if not fc:
                        fc = fcurves.new(eye_L_dp, index=i, action_group=self.eye_L_bone.name)
                    self._anim_values_to_keyframes(fc, self.animation_timestamps, anim_values)
                    keyframes_added = True
            # Right Eye Rotation
            if self.eye_R_bone:
                ...
                for i in range(rot_channel_count_R):
                    ...
                    if self.use_smoothing_eye_bones:
                        if self.smooth_filter_eye_bones == 'SMA':
                            ...
                            anim_values = gaussian_filter1d(anim_values, self.smooth_window_eye_bones, 2)
                    fc = fcurves.find(eye_R_dp, index=i)
                    if not fc:
                        fc = fcurves.new(eye_R_dp, index=i, action_group=self.eye_R_bone.name)
                    self._anim_values_to_keyframes(fc, self.animation_timestamps, anim_values)
                    keyframes_added = True
...
class MocapImporterBase(FaceRegionsBaseProperties, SmoothBaseProperties):
    ...
    def _get_action(self, action_name, replace=False, slot_id: str = None, slot_name: str = None):
        ...
        if not action:
            ...
            if bpy.app.version >= (4, 4, 0):
                action.slots.new(slot_id, slot_name)
        return action
    ...
    def execute(self, context: Context):
        ...
        if animate_shapes or self.animate_eye_rotation_shapes:
            ...
            if self.bake_to_control_rig:
                ...
                if not c_rig:
                    ...
                    return {'CANCELLED'}
                # Get target action
                mocap_action = self._get_action("mocap_import", replace=True, slot_id='KEY', slot_name="ShapeKeys")
                ...
                if scene.faceit_mocap_action is None:  # or self.overwrite_method == 'REPLACE':
                    mocap_action = self._get_action(self.new_action_name, replace=False,
                                                    slot_id='KEY', slot_name="ShapeKeys")
                else:
                    mocap_action = scene.faceit_mocap_action

                mocap_action.use_fake_user = True
                # Get target objects and shapes
            ...
            if bpy.app.version >= (4, 4, 0):
                # get the mocap slot
                if mocap_action is scene.faceit_mocap_action:
                    slot_handle = scene.faceit_mocap_slot_handle
                    if slot_handle:
                        for slot in mocap_action.slots:
                            if slot.handle == slot_handle:
                                mocap_slot = slot
                                break
                if mocap_slot is None:
                    mocap_slot = mocap_action.slots.get("KEShapeKeys")
                    if mocap_slot is None:
                        mocap_slot = mocap_action.slots.new(id_type='KEY', name="ShapeKeys")
                scene.faceit_mocap_slot_handle = mocap_slot.handle
            ...
            if all_set_target_shapes.intersection(available_shape_keys):
                ...
                mocap_importer.set_sk_action(mocap_action, slot=mocap_slot)
            else:
        ...
        if animate_loc or animate_rot:
            ...
            if head_obj:
                ...
                head_slot = None
                adt = head_obj.animation_data
                if adt:
                    head_action = adt.action
                if head_action is None:
                    head_action = bpy.data.actions.new(name=head_obj.name + "Action")
                if bpy.app.version >= (4, 4, 0):
                    if adt:
                        head_slot = adt.action_slot
                    if head_slot is None:
                        head_slot = head_action.slots.new(id_type='OBJECT', name=head_obj.name)
                head_action.use_fake_user = True
                if adt is None:
                    adt = head_obj.animation_data_create()

                adt.action = head_action
                if head_slot:
                    adt.action_slot = head_slot
                mocap_importer.set_head_action(head_action, slot=head_slot)
            if saved_pose:
                restore_saved_pose(head_obj, saved_pose)
        if self.animate_eye_rotation_bones:
            saved_pose = None
            eye_rig: Object = context.scene.faceit_eye_target_rig
            if eye_rig is not None:
                ...
                if eye_R_bone or eye_L_bone:
                    ...
                    if eye_rig is not head_obj or head_action is None:
                        adt = eye_rig.animation_data
                        if adt:
                            eye_action = adt.action
                        if eye_action is None:
                            eye_action = bpy.data.actions.new(name=eye_rig.name + "Action")
                        if bpy.app.version >= (4, 4, 0):
                            eye_slot = adt.action_slot
                            if eye_slot is None:
                                eye_slot = eye_action.slots.new(id_type='OBJECT', name=eye_rig.name)
                        eye_action.use_fake_user = True
                        if adt is None:
                            adt = eye_rig.animation_data_create()

                        adt.action = eye_action
                        if eye_slot:
                            adt.action_slot = eye_slot
                        mocap_importer.set_eye_action(eye_action, eye_slot)
                    else:
                        mocap_importer.set_eye_action(head_action, head_slot)
                else:
                    mocap_importer.animate_eye_bones = False
            if saved_pose:
                restore_saved_pose(eye_rig, saved_pose)
        # Process & Import Animation
        mocap_importer.mix_mode = self.overwrite_method
        ...
        if animate_shapes:
            if self.bake_to_control_rig:
                if mocap_action.fcurves:
                    ...
                    if bpy.app.version >= (4, 4, 0):
                        current_shape_slot = find_slot_by_handle(mocap_action, scene.faceit_mocap_slot_handle)
                    scene.faceit_mocap_action = mocap_action
                    try:
