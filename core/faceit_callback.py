...
def faceit_switch_modes_callback(context):
    ...
    if obj.name == "facial_landmarks":
        if obj.mode == 'EDIT':
            if obj["state"] == 3 and context.preferences.addons[base_package].preferences.auto_lock_3d_view:
...
def faceit_active_object_callback(context):
    ...
    if active_object.name == "facial_landmarks":
        if bpy.context.preferences.addons[base_package].preferences.use_vertex_size_scaling:
            bpy.context.preferences.themes[0].view_3d.vertex_size = bpy.context.preferences.addons[base_package].preferences.landmarks_vertex_size
    else:
        if bpy.context.preferences.addons[base_package].preferences.use_vertex_size_scaling:
            bpy.context.preferences.themes[0].view_3d.vertex_size = bpy.context.preferences.addons[base_package].preferences.default_vertex_size
