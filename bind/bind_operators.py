    ...
    def execute(self, context):
        ...
        for obj, dup_obj in dup_objects_dict.items():
            ...
            if sk_data_dict:
                ...
                if getattr(sk_id, 'key_blocks', None):
                    for sk in sk_id.key_blocks:
                        if sk.name.startswith('faceit_cc_'):
                            sk.mute = False
            if shape_key_utils.has_shape_keys(obj):
                if sk_action:
                    anim_data = sk_id.animation_data
                    if not anim_data:
                        anim_data = sk_id.animation_data_create()
                    anim_data.action = sk_action
