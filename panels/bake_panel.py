...
class FACEIT_PT_ShapeKeyUtils(FACEIT_PT_BaseBake, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column()
        row = col.row(align=True)
        row.prop(context.preferences.addons[base_package].preferences, 'dynamic_shape_key_ranges', icon='SHAPEKEY_DATA')

        if not context.preferences.addons[base_package].preferences.dynamic_shape_key_ranges:
            row = col.row(align=True)
