...
from ..animate.anim_utils import get_active_animation_fcurves_from_id
...
def add_expression_keyframes(rig, frame):
    ...
    fcurves = get_active_animation_fcurves_from_id(rig)
    for fc in fcurves:
...
def create_overwrite_animation(rig_obj: Object) -> Action:
    '''
    ...
    anim_data = rig_obj.animation_data
    if not anim_data:
        anim_data = rig_obj.animation_data_create()
    anim_data.action = ow_action
    if bpy.app.version >= (4, 4, 0):
        if anim_data.action_suitable_slots:
            anim_data.action_slot = anim_data.action_suitable_slots[0]
        else:
            anim_data.action_slot = ow_action.slots.new(id_type='OBJECT', name=rig_obj.name)
    return ow_action
